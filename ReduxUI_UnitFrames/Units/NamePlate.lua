...
UF.UpdateTargetIndicator = function(self)
    if self.unit and UnitIsUnit(self.unit, "target") then
        if R.config.db.profile.modules.unitFrames.nameplates.targetGlow then
            self.TargetIndicator.Shadow:SetOutside(self.Health, 4, 4)
        ...
        if R.config.db.profile.modules.unitFrames.nameplates.targetArrows then
