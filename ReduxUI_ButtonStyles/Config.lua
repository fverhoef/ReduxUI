...
R.config.defaults.profile.modules.buttons = {
    enabled = true,
...
R.config.options.args.buttons = {    
    ...
    args = {
        ...
        enabled = {
            ...
            order = 1,
            confirm = function()
                if R.config.db.profile.modules.buttons.enabled then
            ...
            get = function()
                return R.config.db.profile.modules.buttons.enabled
            end,
            set = function(_, val)
                R.config.db.profile.modules.buttons.enabled = val
                if not val then
                    ReloadUI()
                else
