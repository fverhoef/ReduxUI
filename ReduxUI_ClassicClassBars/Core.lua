...
function CB:Initialize()    
    ...
    if LibStub("Masque", true) then
        mageMasqueGroup = LibStub("Masque", true):Group(R.title, "Mage Bar", true)
    end

    local shamanMasqueGroup
    if LibStub("Masque", true) then
        shamanMasqueGroup = LibStub("Masque", true):Group(R.title, "Shaman Bar", true)
    end

    if R.PlayerClass == "MAGE" then
        ...
        CB:OnUpdate()
    elseif R.PlayerClass == "SHAMAN" then
...
function CB:OnUpdate()
    if CB.MageBar then
        CB:DockClassBar(CB.MageBar, R.config.db.profile.modules.classicClassBars.mageBar.dock)
    elseif CB.ShamanBar then
        CB:DockClassBar(CB.ShamanBar, R.config.db.profile.modules.classicClassBars.shamanBar.dock)
    end
end

function CB:CreateMageBar(masqueGroup)
    local config = R.config.db.profile.modules.classicClassBars.mageBar
    ...
    frame.buttons[1] = CB:CreateSpellFlyout("Teleports", frame, {
        ...
        direction = "UP",
        actions = CB.database.Mage.Teleports[R.PlayerFaction],
    ...
    frame.buttons[2] = CB:CreateSpellFlyout("Portals", frame, {
        size = config.buttonSize,
        direction = "UP",
        actions = CB.database.Mage.Portals[R.PlayerFaction],
        defaultActionIndex = 1,
        showOnlyMaxRank = false,
        masqueGroup = masqueGroup
    })
...
function CB:CreateShamanBar(masqueGroup)
    local config = R.config.db.profile.modules.classicClassBars.shamanBar
...
function CB:UpdateClassBar(bar)
    ...
    if bar.faderConfig then
        R:CreateButtonFrameFader(bar, buttonList, bar.faderConfig)
    end
end
