...
function S:StylePaperDollFrame()
    ...
    S:SecureHook("PaperDollFrame_SetGuild", function()
        ...
        if guildName then
            CharacterGuildText:SetFormattedText(GUILD_TITLE_TEMPLATE, title, R:Hex(R.config.db.profile.modules.skins.colors.guild) .. guildName .. "|r")
        end
    end)

    -- register item slots with Masque
    if LibStub("Masque", true) then
        local masqueGroup = LibStub("Masque", true):Group(R.title, "Character Frame", true)
...
function S:CreateCharacterStatsPane()
    ...
    frame.ClassBackground.Background:SetAllPoints()
    -- TODO: set image based on class
    frame.ClassBackground.Background:SetTexture([[Interface\PaperDollInfoFrame\PaperDollInfoPart1]])
    frame.ClassBackground.Background:SetTexCoord(0 / 1024, 199 / 1024, 0 / 1024, 357 / 1024)
...
function S:CreateCharacterStatsPane_AttributeEntry(attribute, frame, parent, showBackground, point)
    ...
    entry.Label:SetTextColor(unpack(R.config.db.profile.modules.skins.colors.normalFont))
    ...
    entry:SetScript("OnEnter", function()
        if entry.tooltip then
            ...
            GameTooltip:SetText(entry.tooltip, unpack(R.config.db.profile.modules.skins.colors.highlightFont))
            if (entry.tooltip2) then
                GameTooltip:AddLine(entry.tooltip2, R.config.db.profile.modules.skins.colors.normalFont[1], R.config.db.profile.modules.skins.colors.normalFont[2],
                                    R.config.db.profile.modules.skins.colors.normalFont[3], true)
            end
            GameTooltip:Show()
        end
    end)
...
function S:CreateCharacterStatsPane_Enhancements(parent)
    ...
    S:CreateCharacterStatsPane_EnhancementEntry("AttackPower", MELEE_ATTACK_POWER .. ":", frame.Melee, true, {"TOP", frame.Melee.Damage, "BOTTOM", 0, 0})
    S:CreateCharacterStatsPane_EnhancementEntry("AttackRating", ATTACK_TOOLTIP .. ":", frame.Melee, false, {"TOP", frame.Melee.AttackPower, "BOTTOM", 0, 0})
    ...
    S:CreateCharacterStatsPane_EnhancementEntry("AttackPower", RANGED_ATTACK_POWER .. ":", frame.Ranged, true, {"TOP", frame.Ranged.Damage, "BOTTOM", 0, 0})
    S:CreateCharacterStatsPane_EnhancementEntry("AttackRating", ATTACK_TOOLTIP .. ":", frame.Ranged, false, {"TOP", frame.Ranged.AttackPower, "BOTTOM", 0, 0})
    ...
    S:CreateCharacterStatsPane_EnhancementEntry("HealingPower", STAT_SPELLHEALING .. ":", frame.Magic, false, {"TOP", frame.Magic, "TOP", 0, 0})
    S:CreateCharacterStatsPane_EnhancementEntry("SpellPower", STAT_SPELLPOWER .. ":", frame.Magic, true, {"TOP", frame.Magic.HealingPower, "BOTTOM", 0, 0})
    S:CreateCharacterStatsPane_EnhancementEntry("SpellCrit", STAT_CRITICAL_STRIKE .. ":", frame.Magic, false, {"TOP", frame.Magic.SpellPower, "BOTTOM", 0, 0})
    ...
    S:CreateCharacterStatsPane_EnhancementEntry("Armor", STAT_ARMOR .. ":", frame.Defenses, false, {"TOP", frame.Defenses, "TOP", 0, 0})
    S:CreateCharacterStatsPane_EnhancementEntry("Dodge", STAT_DODGE .. ":", frame.Defenses, true, {"TOP", frame.Defenses.Armor, "BOTTOM", 0, 0})
    S:CreateCharacterStatsPane_EnhancementEntry("Parry", STAT_PARRY .. ":", frame.Defenses, false, {"TOP", frame.Defenses.Dodge, "BOTTOM", 0, 0})
    S:CreateCharacterStatsPane_EnhancementEntry("Block", STAT_BLOCK .. ":", frame.Defenses, true, {"TOP", frame.Defenses.Parry, "BOTTOM", 0, 0})
    S:CreateCharacterStatsPane_EnhancementEntry("Defense", DEFENSE .. ":", frame.Defenses, false, {"TOP", frame.Defenses.Block, "BOTTOM", 0, 0})

    frame.Categories = {frame.Melee, frame.Ranged, frame.Magic, frame.Defenses}
    frame.CurrentCategory = R.config.db.profile.modules.skins.enhancementIndex -- TODO: store in cache
    frame.UpdateCategoryVisibility = function()
        ...
        R.config.db.profile.modules.skins.enhancementIndex = frame.CurrentCategory
    end
                ...
                if (line) then
                    if type(line) == "table" then
                        ...
                        elseif type(left) == "table" and type(right) == "string" then
                            ...
                            leftColor = left.color
                            rightText = right
                            rightColor = R.config.db.profile.modules.skins.colors.normalFont
                        elseif type(left) == "string" and type(right) == "table" then
                            leftText = left
                            leftColor = R.config.db.profile.modules.skins.colors.normalFont
                            rightText = right.text
                            rightColor = right.color
                        else
                            leftText = left
                        ...
                        GameTooltip:AddDoubleLine(leftText, rightText, leftColor[1], leftColor[2], leftColor[3], rightColor[1], rightColor[2], rightColor[3])
                    else
                        GameTooltip:AddLine(line, R.config.db.profile.modules.skins.colors.normalFont[1], R.config.db.profile.modules.skins.colors.normalFont[2],
                                            R.config.db.profile.modules.skins.colors.normalFont[3], true)
                    end
                end
            end
...
function S:FormatStatTooltip(name, stat, posBuff, negBuff)
    local tooltip = R:Hex(R.config.db.profile.modules.skins.colors.highlightFont) .. name .. " "
    ...
    if (posBuff == 0 and negBuff == 0) then
        ...
        if (posBuff > 0) then
            tooltip = tooltip .. FONT_COLOR_CODE_CLOSE .. R:Hex(R.config.db.profile.modules.skins.colors.greenFont) .. "+" .. posBuff .. FONT_COLOR_CODE_CLOSE
        end
        if (negBuff < 0) then
            tooltip = tooltip .. R:Hex(R.config.db.profile.modules.skins.colors.redFont) .. " " .. negBuff .. FONT_COLOR_CODE_CLOSE
        end
        if (posBuff > 0 or negBuff < 0) then
            tooltip = tooltip .. R:Hex(R.config.db.profile.modules.skins.colors.highlightFont) .. ")" .. FONT_COLOR_CODE_CLOSE
        end
    end
...
function S:UpdateAttribute(entry, attribute)
    ...
    if effectiveBuff == 0 then
        entry.Value:SetTextColor(unpack(R.config.db.profile.modules.skins.colors.highlightFont))
    elseif effectiveBuff > 0 then
        entry.Value:SetTextColor(unpack(R.config.db.profile.modules.skins.colors.greenFont))
    elseif effectiveBuff < 0 then
        entry.Value:SetTextColor(unpack(R.config.db.profile.modules.skins.colors.redFont))
    end

    if entry.attributeIndex then
        local tooltip = S:FormatStatTooltip(_G["SPELL_STAT" .. entry.attributeIndex .. "_NAME"], attribute.stat, attribute.posBuff, attribute.negBuff)

        -- set class specific tooltip for that stat
        local tooltip2 = _G[S.Stats.class .. "_" .. entry.attributeName .. "_TOOLTIP"]
...
function S:UpdateDamage(entry, damage, mainTooltip)
    ...
    local colorPos = R:Hex(R.config.db.profile.modules.skins.colors.greenFont)
    local colorNeg = R:Hex(R.config.db.profile.modules.skins.colors.redFont)

    local damageText
    -- NOTE: wands don't gain damage modifiers (bug in default UI)
    if S.Stats.hasWand then
    ...
    elseif (totalBonus == 0) then
        ...
        if (damage.percent > 1) then
            damageTooltip = damageTooltip .. colorPos .. " x" .. math.floor(damage.percent * 100 + 0.5) .. "%" .. FONT_COLOR_CODE_CLOSE
        elseif (damage.percent < 1) then
            damageTooltip = damageTooltip .. colorNeg .. " x" .. math.floor(damage.percent * 100 + 0.5) .. "%" .. FONT_COLOR_CODE_CLOSE
        end
    end
    ...
    entry.tooltip2 = {
        left = {text = ATTACK_SPEED_COLON, color = R.config.db.profile.modules.skins.colors.normalFont},
        right = {text = string.format("%.2f", damage.speed or damage.mainhandSpeed), color = R.config.db.profile.modules.skins.colors.highlightFont}
    }
    entry.tooltip3 = {
        left = {text = DAMAGE_COLON, color = R.config.db.profile.modules.skins.colors.normalFont},
        right = {text = damageTooltip, color = R.config.db.profile.modules.skins.colors.highlightFont}
    }
    entry.tooltip4 = {
        left = {text = DAMAGE_PER_SECOND, color = R.config.db.profile.modules.skins.colors.normalFont},
        right = {text = string.format("%.1f", damagePerSecond), color = R.config.db.profile.modules.skins.colors.highlightFont}
    }
    -- entry.tooltip6 = {left = {text = ATTACK_TOOLTIP .. ":", color = R.config.db.profile.modules.skins.colors.normalFont}, right = {text = attackRating or 0, color = R.config.db.profile.modules.skins.colors.highlightFont}}
end
...
function S:UpdateMeleeDamage(entry, melee)
    ...
    if melee.offhandSpeed then
        ...
        local offhandDamageTooltip = math.max(math.floor(minOffHandDamage), 1) .. " - " .. math.max(math.ceil(maxOffHandDamage), 1)
        ...
        if (percent > 1) then
            offhandDamageTooltip = offhandDamageTooltip .. colorPos .. " x" .. math.floor(melee.percent * 100 + 0.5) .. "%" .. FONT_COLOR_CODE_CLOSE
        elseif (percent < 1) then
            offhandDamageTooltip = offhandDamageTooltip .. colorNeg .. " x" .. math.floor(melee.percent * 100 + 0.5) .. "%" .. FONT_COLOR_CODE_CLOSE
        end

        entry.tooltip6 = " "
        entry.tooltip7 = {
            left = {text = ATTACK_SPEED_COLON, color = R.config.db.profile.modules.skins.colors.normalFont},
            right = {text = string.format("%.2f", melee.offhandSpeed), color = R.config.db.profile.modules.skins.colors.highlightFont}
        }
        entry.tooltip8 = {
            left = {text = DAMAGE_COLON, color = R.config.db.profile.modules.skins.colors.normalFont},
            right = {text = offhandDamageTooltip, color = R.config.db.profile.modules.skins.colors.highlightFont}
        }
        entry.tooltip9 = {
            left = {text = DAMAGE_PER_SECOND, color = R.config.db.profile.modules.skins.colors.normalFont},
            right = {text = string.format("%.1f", offhandDamagePerSecond), color = R.config.db.profile.modules.skins.colors.highlightFont}
        }
    end
end
...
function S:UpdateMeleeAttackRating(entry, melee)
    ...
    if melee.attackRating.mainhandModifier ~= 0 then
        ...
        local sign
        if melee.attackRating.mainhandModifier > 0 then
            color = R.config.db.profile.modules.skins.colors.greenFont
            sign = "+"
        else
            color = R.config.db.profile.modules.skins.colors.redFont
            sign = "-"
        end
        entry.tooltip =
            entry.tooltip .. "(" .. melee.attackRating.mainhandBase .. R:Hex(R.config.db.profile.modules.skins.colors.normalFont) .. sign .. melee.attackRating.mainhandModifier .. "|r" ..
                ")"
    end
...
function S:UpdateMeleeCrit(entry, melee)
    ...
    local extraWeaponSkill = S.Stats.weaponSkill - 300
    local bossDefense = 315 -- level 63
    local skillBossDelta = bossDefense - S.Stats.weaponSkill
    local dodgeChance = 5 + (skillBossDelta * 0.1)
    local critCap = 100 - S.Stats.melee.miss.missChanceVsBoss - dodgeChance - glancingChance + critSuppression + (extraWeaponSkill * 0.04)
    ...
    if offhandItemId then
        local critCapDw = 100 - S.Stats.melee.miss.dwMissChanceVsBoss - dodgeChance - glancingChance + critSuppression + (extraWeaponSkill * 0.04);
        ...
        entry.tooltip5 = {left = string.format("    Level 63 NPC/Boss: %s", critCapTxt), right = string.format("(Dual wield: %s)", critCapDwTxt)}
    else
...
function S:UpdateMeleeHit(entry, melee)
    ...
    entry.tooltip5 = {
        left = string.format("    Level 60 NPC: %.2f%%", S.Stats.melee.miss.missChanceVsNPC),
        right = string.format("(Dual wield: %.2f%%)", S.Stats.melee.miss.dwMissChanceVsNpc)
    }
    entry.tooltip6 = {
        left = string.format("    Level 60 Player: %.2f%%", S.Stats.melee.miss.missChanceVsPlayer),
        right = string.format("(Dual wield: %.2f%%)", S.Stats.melee.miss.dwMissChanceVsPlayer)
    }
    entry.tooltip7 = {
        left = string.format("    Level 63 NPC/Boss: %.2f%%", S.Stats.melee.miss.missChanceVsBoss),
        right = string.format("(Dual wield: %.2f%%)", S.Stats.melee.miss.dwMissChanceVsBoss)
    }
end
...
function S:UpdateRangedAttackPower(entry, ranged)
    if not S.Stats.hasRangedWeapon or S.Stats.hasWand or not ranged.attackPower.base then
...
function S:UpdateRangedAttackRating(entry, ranged)
    if not S.Stats.hasRangedWeapon then
        entry.Value:SetText(NOT_APPLICABLE)
        ...
        if ranged.attackRating.modifier ~= 0 then
            ...
            entry.tooltip =
                entry.tooltip .. "(" .. ranged.attackRating.base .. R:Hex(R.config.db.profile.modules.skins.colors.normalFont) .. sign .. ranged.attackRating.modifier .. "|r" .. ")"
        end
        entry.tooltip2 = ATTACK_TOOLTIP_SUBTEXT
    end
end
...
function S:UpdateHealingPower(entry, healing)
    ...
    if S.Stats.class == "SHAMAN" then
        if S.Stats.lesserHealingWave > 0 then
            entry.tooltip2 = "Lesser Healing Wave: " .. (healing + S.Stats.lesserHealingWave) -- TODO: Localization
            index = index + 1
        end
    end
...
function S:UpdateSpellPower(entry, spellPower)
    ...
    entry.tooltip3 = " "
    entry.tooltip4 = {
        left = {text = SPELL_SCHOOL1_CAP .. ": ", color = R.config.db.profile.modules.skins.colors.holy},
        right = {text = string.format("%i", spellPower.holy), color = R.config.db.profile.modules.skins.colors.holy}
    }
    entry.tooltip5 = {
        left = {text = SPELL_SCHOOL2_CAP .. ": ", color = R.config.db.profile.modules.skins.colors.fire},
        right = {text = string.format("%i", spellPower.fire), color = R.config.db.profile.modules.skins.colors.fire}
    }
    entry.tooltip6 = {
        left = {text = SPELL_SCHOOL4_CAP .. ": ", color = R.config.db.profile.modules.skins.colors.frost},
        right = {text = string.format("%i", spellPower.frost), color = R.config.db.profile.modules.skins.colors.frost}
    }
    entry.tooltip7 = {
        left = {text = SPELL_SCHOOL6_CAP .. ": ", color = R.config.db.profile.modules.skins.colors.arcane},
        right = {text = string.format("%i", spellPower.arcane), color = R.config.db.profile.modules.skins.colors.arcane}
    }
    entry.tooltip8 = {
        left = {text = SPELL_SCHOOL5_CAP .. ": ", color = R.config.db.profile.modules.skins.colors.shadow},
        right = {text = string.format("%i", spellPower.shadow), color = R.config.db.profile.modules.skins.colors.shadow}
    }
    entry.tooltip9 = {
        left = {text = SPELL_SCHOOL3_CAP .. ": ", color = R.config.db.profile.modules.skins.colors.nature},
        right = {text = string.format("%i", spellPower.nature), color = R.config.db.profile.modules.skins.colors.nature}
    }
    if S.Stats.class == "SHAMAN" then
        if spellPower.lightning > 0 then
            entry.tooltip10 = {
                left = {text = "   Lightning: ", color = R.config.db.profile.modules.skins.colors.nature}, -- TODO: Localization
                right = {text = string.format("%i", spellPower.nature + spellPower.lightning), color = R.config.db.profile.modules.skins.colors.nature}
            }
        elseif spellPower.shocks > 0 then
            entry.tooltip10 = {
                left = {text = "   Flame Shock: ", color = R.config.db.profile.modules.skins.colors.fire}, -- TODO: Localization
                right = {text = string.format("%i", spellPower.fire + spellPower.shocks), color = R.config.db.profile.modules.skins.colors.fire}
            }
            entry.tooltip11 = {
                left = {text = "   Frost Shock: ", color = R.config.db.profile.modules.skins.colors.frost}, -- TODO: Localization
                right = {text = string.format("%i", spellPower.frost + spellPower.shocks), color = R.config.db.profile.modules.skins.colors.frost}
            }
            entry.tooltip12 = {
                left = {text = "   Earth Shock: ", color = R.config.db.profile.modules.skins.colors.nature}, -- TODO: Localization
                right = {text = string.format("%i", spellPower.nature + spellPower.shocks), color = R.config.db.profile.modules.skins.colors.nature}
            }
        end
    end
end
...
function S:UpdateSpellCrit(entry, spellCrit)
    ...
    entry["tooltip" .. index] = {
        left = {text = SPELL_SCHOOL1_CAP .. ":", color = R.config.db.profile.modules.skins.colors.holy},
        right = {text = string.format("%.2f%%", spellCrit.holy), color = R.config.db.profile.modules.skins.colors.holy}
    }
    index = index + 1

    entry["tooltip" .. index] = {
        left = {text = SPELL_SCHOOL2_CAP .. ":", color = R.config.db.profile.modules.skins.colors.fire},
        right = {text = string.format("%.2f%%", spellCrit.fire), color = R.config.db.profile.modules.skins.colors.fire}
    }
    index = index + 1

    if S.Stats.class == "WARLOCK" then
        entry["tooltip" .. index] = {
            left = {text = "   Searing Pain:", color = R.config.db.profile.modules.skins.colors.fire},
            right = {text = string.format("%.2f%%", spellCrit.searingPain), color = R.config.db.profile.modules.skins.colors.fire}
        }
        index = index + 1
    end
    ...
    entry["tooltip" .. index] = {
        left = {text = SPELL_SCHOOL4_CAP .. ":", color = R.config.db.profile.modules.skins.colors.frost},
        right = {text = string.format("%.2f%%", spellCrit.frost), color = R.config.db.profile.modules.skins.colors.frost}
    }
    index = index + 1

    entry["tooltip" .. index] = {
        left = {text = SPELL_SCHOOL6_CAP .. ":", color = R.config.db.profile.modules.skins.colors.arcane},
        right = {text = string.format("%.2f%%", spellCrit.arcane), color = R.config.db.profile.modules.skins.colors.arcane}
    }
    index = index + 1

    entry["tooltip" .. index] = {
        left = {text = SPELL_SCHOOL5_CAP .. ":", color = R.config.db.profile.modules.skins.colors.shadow},
        right = {text = string.format("%.2f%%", spellCrit.shadow), color = R.config.db.profile.modules.skins.colors.shadow}
    }
    index = index + 1

    if S.Stats.class == "WARLOCK" then
        entry["tooltip" .. index] = {
            left = {text = "   Destruction:", color = R.config.db.profile.modules.skins.colors.shadow},
            right = {text = string.format("%.2f%%", spellCrit.destruction), color = R.config.db.profile.modules.skins.colors.shadow}
        }
        index = index + 1
    end
    ...
    entry["tooltip" .. index] = {
        left = {text = SPELL_SCHOOL3_CAP .. ":", color = R.config.db.profile.modules.skins.colors.nature},
        right = {text = string.format("%.2f%%", spellCrit.nature), color = R.config.db.profile.modules.skins.colors.nature}
    }
    index = index + 1

    if S.Stats.class == "SHAMAN" then
        entry["tooltip" .. index] = {
            left = {text = "   Healing:", color = R.config.db.profile.modules.skins.colors.nature},
            right = {text = string.format("%.2f%%", spellCrit.healing), color = R.config.db.profile.modules.skins.colors.nature}
        }
        index = index + 1

        entry["tooltip" .. index] = {
            left = {text = "   Lightning:", color = R.config.db.profile.modules.skins.colors.nature},
            right = {text = string.format("%.2f%%", spellCrit.lightning), color = R.config.db.profile.modules.skins.colors.nature}
        }
    end
end
...
function S:UpdateSpellHit(entry, spellHit)
    ...
    entry.tooltip4 = {
        left = {text = SPELL_SCHOOL1_CAP .. ": ", color = R.config.db.profile.modules.skins.colors.holy},
        right = {text = string.format("%i%%", spellHit.holy), color = R.config.db.profile.modules.skins.colors.holy}
    }
    entry.tooltip5 = {
        left = {text = SPELL_SCHOOL2_CAP .. ": ", color = R.config.db.profile.modules.skins.colors.fire},
        right = {text = string.format("%i%%", spellHit.fire), color = R.config.db.profile.modules.skins.colors.fire}
    }
    entry.tooltip6 = {
        left = {text = SPELL_SCHOOL4_CAP .. ": ", color = R.config.db.profile.modules.skins.colors.frost},
        right = {text = string.format("%i%%", spellHit.frost), color = R.config.db.profile.modules.skins.colors.frost}
    }
    entry.tooltip7 = {
        left = {text = SPELL_SCHOOL6_CAP .. ": ", color = R.config.db.profile.modules.skins.colors.arcane},
        right = {text = string.format("%i%%", spellHit.arcane), color = R.config.db.profile.modules.skins.colors.arcane}
    }
    entry.tooltip8 = {
        left = {text = SPELL_SCHOOL5_CAP .. ": ", color = R.config.db.profile.modules.skins.colors.shadow},
        right = {text = string.format("%i%%", spellHit.shadow), color = R.config.db.profile.modules.skins.colors.shadow}
    }
    entry.tooltip9 = {
        left = {text = SPELL_SCHOOL3_CAP .. ": ", color = R.config.db.profile.modules.skins.colors.nature},
        right = {text = string.format("%i%%", spellHit.nature), color = R.config.db.profile.modules.skins.colors.nature}
    }
end
...
function S:UpdateManaRegen(entry, regen)
    ...
    entry.tooltip4 = {
        ...
        right = {text = string.format("%i", regen.base * 5), color = R.config.db.profile.modules.skins.colors.highlightFont}
    }
    entry.tooltip5 = {
        left = string.format("%s While Casting:", MANA_REGEN),
        right = {text = string.format("%i", regen.casting * 5), color = R.config.db.profile.modules.skins.colors.highlightFont}
    }
end
...
function S:UpdateArmor(entry, armor)
    ...
    entry.tooltip2 = string.format(ARMOR_TOOLTIP, S.Stats.level, S.Stats.armorReduction)
end
...
function S:UpdateBlock(entry, block, blockValue)
    ...
    entry.tooltip3 = " "
    entry.tooltip4 = {left = BLOCK_CHANCE .. ": ", right = {text = string.format("%.2f%%", block), color = R.config.db.profile.modules.skins.colors.highlightFont}}
    entry.tooltip5 = {left = ITEM_MOD_BLOCK_VALUE_SHORT .. ": ", right = {text = string.format("%i", blockValue), color = R.config.db.profile.modules.skins.colors.highlightFont}}
end
...
function S:UpdatePaperDollFrame()
    ...
    if S.Stats.itemLevel then
        if S.Stats.itemLevel >= R.config.db.profile.modules.skins.itemLevels.legendary then
            itemLevelCategory = 5 -- legendary
        elseif S.Stats.itemLevel >= R.config.db.profile.modules.skins.itemLevels.epic then
            itemLevelCategory = 4 -- epic
        elseif S.Stats.itemLevel >= R.config.db.profile.modules.skins.itemLevels.rare then
            itemLevelCategory = 3 -- rare
        elseif S.Stats.itemLevel >= R.config.db.profile.modules.skins.itemLevels.uncommon then
            itemLevelCategory = 2 -- uncommon
        elseif S.Stats.itemLevel >= R.config.db.profile.modules.skins.itemLevels.common then
            itemLevelCategory = 1 -- common
        else
            itemLevelCategory = 0 -- poor
        end
        local r, g, b = GetItemQualityColor(itemLevelCategory)
        PaperDollFrame.CharacterStatsPane.ItemLevel.Value.Text:SetText(R:Hex(r, g, b) .. S.Stats.itemLevel .. "|r")
    end

    -- update attributes
    S:UpdateAttribute(PaperDollFrame.CharacterStatsPane.Attributes.Strength, S.Stats.strength)
    S:UpdateAttribute(PaperDollFrame.CharacterStatsPane.Attributes.Agility, S.Stats.agility)
    S:UpdateAttribute(PaperDollFrame.CharacterStatsPane.Attributes.Stamina, S.Stats.stamina)
    S:UpdateAttribute(PaperDollFrame.CharacterStatsPane.Attributes.Intellect, S.Stats.intellect)
    S:UpdateAttribute(PaperDollFrame.CharacterStatsPane.Attributes.Spirit, S.Stats.spirit)

    -- update enhancements: melee
    S:UpdateMeleeDamage(PaperDollFrame.CharacterStatsPane.Enhancements.Melee.Damage, S.Stats.melee)
    S:UpdateMeleeAttackPower(PaperDollFrame.CharacterStatsPane.Enhancements.Melee.AttackPower, S.Stats.melee)
    S:UpdateMeleeAttackRating(PaperDollFrame.CharacterStatsPane.Enhancements.Melee.AttackRating, S.Stats.melee)
    S:UpdateMeleeCrit(PaperDollFrame.CharacterStatsPane.Enhancements.Melee.Crit, S.Stats.melee)
    S:UpdateMeleeHit(PaperDollFrame.CharacterStatsPane.Enhancements.Melee.Hit, S.Stats.melee)

    -- update enhancements: ranged
    S:UpdateRangedDamage(PaperDollFrame.CharacterStatsPane.Enhancements.Ranged.Damage, S.Stats.ranged)
    S:UpdateRangedAttackPower(PaperDollFrame.CharacterStatsPane.Enhancements.Ranged.AttackPower, S.Stats.ranged)
    S:UpdateRangedAttackRating(PaperDollFrame.CharacterStatsPane.Enhancements.Ranged.AttackRating, S.Stats.ranged)
    S:UpdateRangedCrit(PaperDollFrame.CharacterStatsPane.Enhancements.Ranged.Crit, S.Stats.ranged)
    S:UpdateRangedHit(PaperDollFrame.CharacterStatsPane.Enhancements.Ranged.Hit, S.Stats.ranged)

    -- update enhancements: magic
    S:UpdateHealingPower(PaperDollFrame.CharacterStatsPane.Enhancements.Magic.HealingPower, S.Stats.healingPower)
    S:UpdateSpellPower(PaperDollFrame.CharacterStatsPane.Enhancements.Magic.SpellPower, S.Stats.spellPower)
    S:UpdateSpellCrit(PaperDollFrame.CharacterStatsPane.Enhancements.Magic.SpellCrit, S.Stats.spellCrit)
    S:UpdateSpellHit(PaperDollFrame.CharacterStatsPane.Enhancements.Magic.SpellHit, S.Stats.spellHit)
    S:UpdateManaRegen(PaperDollFrame.CharacterStatsPane.Enhancements.Magic.ManaRegen, S.Stats.manaRegen)

    -- update enhancements: defenses
    S:UpdateArmor(PaperDollFrame.CharacterStatsPane.Enhancements.Defenses.Armor, S.Stats.armor)
    S:UpdateDodge(PaperDollFrame.CharacterStatsPane.Enhancements.Defenses.Dodge, S.Stats.dodge)
