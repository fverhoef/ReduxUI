...
function Addon:UpdateBlizzardFonts()
    DAMAGE_TEXT_FONT = Addon.config.db.profile.fonts.damage
    STANDARD_TEXT_FONT = Addon.config.db.profile.fonts.normal
    UNIT_NAME_FONT = Addon.config.db.profile.fonts.unitName

    for _, font in next, blizzardFonts.normal do
        if font then
            local fontFace, height, flags = font:GetFont()
            font:SetFont(Addon.config.db.profile.fonts.normal, height, flags)
        end
    end
    ...
    for _, font in next, blizzardFonts.number do
        if font then
            local fontFace, height, flags = font:GetFont()
            font:SetFont(Addon.config.db.profile.fonts.number, height, flags)
        end
    end
    ...
    for _, font in next, blizzardFonts.chatBubble do
        if font then
            local fontFace, height, flags = font:GetFont()
            font:SetFont(Addon.config.db.profile.fonts.chatBubble, height, flags)
        end
    end
end
