...
function B:DisableBlizzardFrames()
    ...
    BankFrame:SetParent(Addon.HiddenFrame)
end
...
function B:UpdateBagSlotButton(button)
    if Addon.Modules.Buttons then
        Addon.Modules.Buttons:StyleBagButton(button)
    end
...
function B:UpdateItemButton(bag, slot)
    ...
    if Addon.Modules.Buttons then
        Addon.Modules.Buttons:StyleItemButton(button)
    end
    ...
    if itemId then
        ...
        if border then
            if itemClassID == LE_ITEM_CLASS_QUESTITEM then
                border:SetVertexColor(unpack(B.config.db.profile.colors.questItem))
                border:SetTexture(Addon.media.textures.buttonBorderWhite)
                border:Show()
            elseif quality > 1 then
                ...
                -- border:SetTexture(Addon.media.textures.buttonBorderWhiteThin)
            end
        end
    end
...
function B:UpdateItemButtons(frame)
    ...
    local config = isBank and B.config.db.profile.bank or B.config.db.profile.inventory
    local column = 1
    local row = 1
...
function B:CreateInventoryFrame()
    ...
    frame.Title:SetText(Addon.PlayerName .. "'s Inventory") -- TODO: localization
    ...
    frame.Money:SetScript("OnEnter", function()
        ...
        for i, char in next, Addon.config.db.realm.inventory do
            if char.money then
                ...
                GameTooltip:AddDoubleLine(Addon:Hex(RAID_CLASS_COLORS[char.class or "MAGE"]) .. i .. "|r:", Addon:FormatMoney(char.money, "FULL", false), 1, 1, 1, 1, 1, 1)
            end
        end
        GameTooltip:AddDoubleLine("Total:", Addon:FormatMoney(total, "FULL", false), 1, 1, 1, 1, 1, 1)
        GameTooltip:Show()
    end)
...
function B:CreateBankFrame()
    ...
    frame.Title:SetPoint("TOP", frame, "TOP", 10, -5)
    frame.Title:SetJustifyH("MIDDLE")
    frame.Title:SetText(Addon.PlayerName .. "'s Bank") -- TODO: localization
