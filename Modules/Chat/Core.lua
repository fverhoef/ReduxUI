...
function C:UpdateChatFrames()
    -- editbox font
    ChatFontNormal:SetFont(unpack(C.config.db.profile.font))

    -- ChatFrameMenuButton
    ChatFrameMenuButton:HookScript("OnShow", function()
        if C.config.db.profile.hideMenuButton then
    ...
    ChatFrameChannelButton:HookScript("OnShow", function()
        if C.config.db.profile.hideChannelButton then
    ...
    if C.config.db.profile.showClassColors then
...
function C:SkinChatFrame(frame)
    ...
    frame:SetFont(unpack(C.config.db.profile.font))
...
function C:StoreChatMessage(frame, text, ...)
    ...
    local count = #db
    local countOverMax = count - C.config.db.profile.maxHistoryCount
