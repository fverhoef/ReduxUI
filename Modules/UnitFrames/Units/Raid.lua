...
function UF:SpawnRaid()
    local config = UF.config.db.profile.raid
    ...
    if config.enabled then
        ...
        for i = 1, NUM_RAID_GROUPS do
            ...
            if config.fader and config.fader.enabled then
                Addon:CreateFrameFader(raidGroup, config.fader)
            end

            Addon:CreateDragFrame(raidGroup, "Raid Group " .. i, default.points[i], 200, 40)
        end
    end
end
...
function UF:CreateRaid()
    ...
    if self.cfg.border.enabled then
        Addon.CreateBorder(self, self.cfg.border.size)
        height = height + 2 * self.cfg.border.size
        width = width + 2 * self.cfg.border.size
        self:SetSize(width, height)
    end
