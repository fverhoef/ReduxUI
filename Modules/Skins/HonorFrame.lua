...
function S:StyleHonorFrame()
    ...
    S:SecureHook("HonorFrame_SetLevel", function()
        ...
        HonorLevelText:SetFormattedText(PLAYER_LEVEL, UnitLevel("player"), R:Hex(1, 1, 1) .. UnitRace("player") .. "|r",
    ...
    S:SecureHook("HonorFrame_SetGuild", function()
        ...
        if guildName then
            HonorGuildText:SetFormattedText(GUILD_TITLE_TEMPLATE, title, R:Hex(S.config.colors.guild) .. guildName .. "|r")
        end
    end)
end
