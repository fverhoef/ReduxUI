...
function S:StylePaperDollFrame()
    ...
    S:SecureHook("PaperDollFrame_SetLevel", function()
        ...
        CharacterLevelText:SetFormattedText(PLAYER_LEVEL, UnitLevel("player"), Addon:Hex(1, 1, 1) .. UnitRace("player") .. "|r", Addon:Hex(classColor) .. UnitClass("player") .. "|r")
    end)
    S:SecureHook("PaperDollFrame_SetGuild", function()
        local guildName, title, rank = GetGuildInfo("player")
        if guildName then
            CharacterGuildText:SetFormattedText(GUILD_TITLE_TEMPLATE, title, Addon:Hex(S.config.db.profile.colors.guild) .. guildName .. "|r")
        end
    end)
...
function S:CreateCharacterStatsPane_AttributeEntry(attribute, frame, parent, showBackground, point)
    ...
    entry.Label:SetTextColor(unpack(S.config.db.profile.colors.normalFont))
    ...
    entry:SetScript("OnEnter", function()
        if entry.tooltip then
            ...
            GameTooltip:SetText(entry.tooltip, unpack(S.config.db.profile.colors.highlightFont))
            if (entry.tooltip2) then
                GameTooltip:AddLine(entry.tooltip2, S.config.db.profile.colors.normalFont[1], S.config.db.profile.colors.normalFont[2],
                                    S.config.db.profile.colors.normalFont[3], true)
            end
            GameTooltip:Show()
        end
    end)
...
function S:CreateCharacterStatsPane_Enhancements(parent)
    ...
    frame.Header.ButtonPrevious:SetNormalTexture(Addon.media.textures.arrowDown_Up)
    frame.Header.ButtonPrevious:SetPushedTexture(Addon.media.textures.arrowDown_Down)
    ...
    frame.Header.ButtonNext:SetNormalTexture(Addon.media.textures.arrowUp_Up)
    frame.Header.ButtonNext:SetPushedTexture(Addon.media.textures.arrowUp_Down)
    ...
    frame.CurrentCategory = S.config.db.profile.enhancementIndex -- TODO: store in cache
    frame.UpdateCategoryVisibility = function()
        ...
        S.config.db.profile.enhancementIndex = frame.CurrentCategory
    end
                ...
                if (line) then
                    if type(line) == "table" then
                        ...
                        elseif type(left) == "table" and type(right) == "string" then
                            ...
                            leftColor = left.color
                            rightText = right
                            rightColor = S.config.db.profile.colors.normalFont
                        elseif type(left) == "string" and type(right) == "table" then
                            leftText = left
                            leftColor = S.config.db.profile.colors.normalFont
                            rightText = right.text
                            rightColor = right.color
                        else
                            leftText = left
                        ...
                        GameTooltip:AddLine(line, S.config.db.profile.colors.normalFont[1], S.config.db.profile.colors.normalFont[2],
                                            S.config.db.profile.colors.normalFont[3], true)
                    end
                end
            end
...
function S:FormatStatTooltip(name, stat, posBuff, negBuff)
    local tooltip = Addon:Hex(S.config.db.profile.colors.highlightFont) .. name .. " "
    ...
    if (posBuff == 0 and negBuff == 0) then
        ...
        if (posBuff > 0) then
            tooltip = tooltip .. FONT_COLOR_CODE_CLOSE .. Addon:Hex(S.config.db.profile.colors.greenFont) .. "+" .. posBuff .. FONT_COLOR_CODE_CLOSE
        end
        if (negBuff < 0) then
            tooltip = tooltip .. Addon:Hex(S.config.db.profile.colors.redFont) .. " " .. negBuff .. FONT_COLOR_CODE_CLOSE
        end
        if (posBuff > 0 or negBuff < 0) then
            tooltip = tooltip .. Addon:Hex(S.config.db.profile.colors.highlightFont) .. ")" .. FONT_COLOR_CODE_CLOSE
        end
    end
...
function S:UpdateAttribute(entry, attribute)
    ...
    if effectiveBuff == 0 then
        entry.Value:SetTextColor(unpack(S.config.db.profile.colors.highlightFont))
    elseif effectiveBuff > 0 then
        entry.Value:SetTextColor(unpack(S.config.db.profile.colors.greenFont))
    elseif effectiveBuff < 0 then
        entry.Value:SetTextColor(unpack(S.config.db.profile.colors.redFont))
    end
...
function S:UpdateDamage(entry, damage, mainTooltip)
    ...
    local colorPos = Addon:Hex(S.config.db.profile.colors.greenFont)
    local colorNeg = Addon:Hex(S.config.db.profile.colors.redFont)
    ...
    entry.tooltip2 = {
        left = {text = ATTACK_SPEED_COLON, color = S.config.db.profile.colors.normalFont},
        right = {text = string.format("%.2f", damage.speed or damage.mainhandSpeed), color = S.config.db.profile.colors.highlightFont}
    }
    entry.tooltip3 = {
        left = {text = DAMAGE_COLON, color = S.config.db.profile.colors.normalFont},
        right = {text = damageTooltip, color = S.config.db.profile.colors.highlightFont}
    }
    entry.tooltip4 = {
        left = {text = DAMAGE_PER_SECOND, color = S.config.db.profile.colors.normalFont},
        right = {text = string.format("%.1f", damagePerSecond), color = S.config.db.profile.colors.highlightFont}
    }
    -- entry.tooltip6 = {left = {text = ATTACK_TOOLTIP .. ":", color = S.config.db.profile.colors.normalFont}, right = {text = attackRating or 0, color = S.config.db.profile.colors.highlightFont}}
end
...
function S:UpdateMeleeDamage(entry, melee)
    ...
    if melee.offhandSpeed then
        ...
        entry.tooltip7 = {
            left = {text = ATTACK_SPEED_COLON, color = S.config.db.profile.colors.normalFont},
            right = {text = string.format("%.2f", melee.offhandSpeed), color = S.config.db.profile.colors.highlightFont}
        }
        entry.tooltip8 = {
            left = {text = DAMAGE_COLON, color = S.config.db.profile.colors.normalFont},
            right = {text = offhandDamageTooltip, color = S.config.db.profile.colors.highlightFont}
        }
        entry.tooltip9 = {
            left = {text = DAMAGE_PER_SECOND, color = S.config.db.profile.colors.normalFont},
            right = {text = string.format("%.1f", offhandDamagePerSecond), color = S.config.db.profile.colors.highlightFont}
        }
    end
end
...
function S:UpdateMeleeAttackRating(entry, melee)
    ...
    if melee.attackRating.mainhandModifier ~= 0 then
        ...
        local sign
        if melee.attackRating.mainhandModifier > 0 then
            color = S.config.db.profile.colors.greenFont
            sign = "+"
        else
            color = S.config.db.profile.colors.redFont
            sign = "-"
        end
        entry.tooltip =
            entry.tooltip .. "(" .. melee.attackRating.mainhandBase .. Addon:Hex(S.config.db.profile.colors.normalFont) .. sign .. melee.attackRating.mainhandModifier .. "|r" ..
                ")"
    end
...
function S:UpdateRangedAttackRating(entry, ranged)
    if not S.Stats.hasRangedWeapon then
        ...
        if ranged.attackRating.modifier ~= 0 then
            ...
            entry.tooltip =
                entry.tooltip .. "(" .. ranged.attackRating.base .. Addon:Hex(S.config.db.profile.colors.normalFont) .. sign .. ranged.attackRating.modifier .. "|r" .. ")"
        end
        entry.tooltip2 = ATTACK_TOOLTIP_SUBTEXT
    end
end
...
function S:UpdateSpellPower(entry, spellPower)
    ...
    entry.tooltip3 = " "
    entry.tooltip4 = {
        left = {text = SPELL_SCHOOL1_CAP .. ": ", color = S.config.db.profile.colors.holy},
        right = {text = string.format("%i", spellPower.holy), color = S.config.db.profile.colors.holy}
    }
    entry.tooltip5 = {
        left = {text = SPELL_SCHOOL2_CAP .. ": ", color = S.config.db.profile.colors.fire},
        right = {text = string.format("%i", spellPower.fire), color = S.config.db.profile.colors.fire}
    }
    entry.tooltip6 = {
        left = {text = SPELL_SCHOOL4_CAP .. ": ", color = S.config.db.profile.colors.frost},
        right = {text = string.format("%i", spellPower.frost), color = S.config.db.profile.colors.frost}
    }
    entry.tooltip7 = {
        left = {text = SPELL_SCHOOL6_CAP .. ": ", color = S.config.db.profile.colors.arcane},
        right = {text = string.format("%i", spellPower.arcane), color = S.config.db.profile.colors.arcane}
    }
    entry.tooltip8 = {
        left = {text = SPELL_SCHOOL5_CAP .. ": ", color = S.config.db.profile.colors.shadow},
        right = {text = string.format("%i", spellPower.shadow), color = S.config.db.profile.colors.shadow}
    }
    entry.tooltip9 = {
        left = {text = SPELL_SCHOOL3_CAP .. ": ", color = S.config.db.profile.colors.nature},
        right = {text = string.format("%i", spellPower.nature), color = S.config.db.profile.colors.nature}
    }
    if S.Stats.class == "SHAMAN" then
        if spellPower.lightning > 0 then
            entry.tooltip10 = {
                left = {text = "   Lightning: ", color = S.config.db.profile.colors.nature}, -- TODO: Localization
                right = {text = string.format("%i", spellPower.nature + spellPower.lightning), color = S.config.db.profile.colors.nature}
            }
        elseif spellPower.shocks > 0 then
            entry.tooltip10 = {
                left = {text = "   Flame Shock: ", color = S.config.db.profile.colors.fire}, -- TODO: Localization
                right = {text = string.format("%i", spellPower.fire + spellPower.shocks), color = S.config.db.profile.colors.fire}
            }
            entry.tooltip11 = {
                left = {text = "   Frost Shock: ", color = S.config.db.profile.colors.frost}, -- TODO: Localization
                right = {text = string.format("%i", spellPower.frost + spellPower.shocks), color = S.config.db.profile.colors.frost}
            }
            entry.tooltip12 = {
                left = {text = "   Earth Shock: ", color = S.config.db.profile.colors.nature}, -- TODO: Localization
                right = {text = string.format("%i", spellPower.nature + spellPower.shocks), color = S.config.db.profile.colors.nature}
            }
        end
    end
end
...
function S:UpdateSpellCrit(entry, spellCrit)
    ...
    entry["tooltip" .. index] = {
        left = {text = SPELL_SCHOOL1_CAP .. ":", color = S.config.db.profile.colors.holy},
        right = {text = string.format("%.2f%%", spellCrit.holy), color = S.config.db.profile.colors.holy}
    }
    index = index + 1

    entry["tooltip" .. index] = {
        left = {text = SPELL_SCHOOL2_CAP .. ":", color = S.config.db.profile.colors.fire},
        right = {text = string.format("%.2f%%", spellCrit.fire), color = S.config.db.profile.colors.fire}
    }
    index = index + 1

    if S.Stats.class == "WARLOCK" then
        entry["tooltip" .. index] = {
            left = {text = "   Searing Pain:", color = S.config.db.profile.colors.fire},
            right = {text = string.format("%.2f%%", spellCrit.searingPain), color = S.config.db.profile.colors.fire}
        }
        index = index + 1
    end
    ...
    entry["tooltip" .. index] = {
        left = {text = SPELL_SCHOOL4_CAP .. ":", color = S.config.db.profile.colors.frost},
        right = {text = string.format("%.2f%%", spellCrit.frost), color = S.config.db.profile.colors.frost}
    }
    index = index + 1

    entry["tooltip" .. index] = {
        left = {text = SPELL_SCHOOL6_CAP .. ":", color = S.config.db.profile.colors.arcane},
        right = {text = string.format("%.2f%%", spellCrit.arcane), color = S.config.db.profile.colors.arcane}
    }
    index = index + 1

    entry["tooltip" .. index] = {
        left = {text = SPELL_SCHOOL5_CAP .. ":", color = S.config.db.profile.colors.shadow},
        right = {text = string.format("%.2f%%", spellCrit.shadow), color = S.config.db.profile.colors.shadow}
    }
    index = index + 1

    if S.Stats.class == "WARLOCK" then
        entry["tooltip" .. index] = {
            left = {text = "   Destruction:", color = S.config.db.profile.colors.shadow},
            right = {text = string.format("%.2f%%", spellCrit.destruction), color = S.config.db.profile.colors.shadow}
        }
        index = index + 1
    end
    ...
    entry["tooltip" .. index] = {
        left = {text = SPELL_SCHOOL3_CAP .. ":", color = S.config.db.profile.colors.nature},
        right = {text = string.format("%.2f%%", spellCrit.nature), color = S.config.db.profile.colors.nature}
    }
    index = index + 1

    if S.Stats.class == "SHAMAN" then
        entry["tooltip" .. index] = {
            left = {text = "   Healing:", color = S.config.db.profile.colors.nature},
            right = {text = string.format("%.2f%%", spellCrit.healing), color = S.config.db.profile.colors.nature}
        }
        index = index + 1

        entry["tooltip" .. index] = {
            left = {text = "   Lightning:", color = S.config.db.profile.colors.nature},
            right = {text = string.format("%.2f%%", spellCrit.lightning), color = S.config.db.profile.colors.nature}
        }
    end
end
...
function S:UpdateSpellHit(entry, spellHit)
    ...
    entry.tooltip4 = {
        left = {text = SPELL_SCHOOL1_CAP .. ": ", color = S.config.db.profile.colors.holy},
        right = {text = string.format("%i%%", spellHit.holy), color = S.config.db.profile.colors.holy}
    }
    entry.tooltip5 = {
        left = {text = SPELL_SCHOOL2_CAP .. ": ", color = S.config.db.profile.colors.fire},
        right = {text = string.format("%i%%", spellHit.fire), color = S.config.db.profile.colors.fire}
    }
    entry.tooltip6 = {
        left = {text = SPELL_SCHOOL4_CAP .. ": ", color = S.config.db.profile.colors.frost},
        right = {text = string.format("%i%%", spellHit.frost), color = S.config.db.profile.colors.frost}
    }
    entry.tooltip7 = {
        left = {text = SPELL_SCHOOL6_CAP .. ": ", color = S.config.db.profile.colors.arcane},
        right = {text = string.format("%i%%", spellHit.arcane), color = S.config.db.profile.colors.arcane}
    }
    entry.tooltip8 = {
        left = {text = SPELL_SCHOOL5_CAP .. ": ", color = S.config.db.profile.colors.shadow},
        right = {text = string.format("%i%%", spellHit.shadow), color = S.config.db.profile.colors.shadow}
    }
    entry.tooltip9 = {
        left = {text = SPELL_SCHOOL3_CAP .. ": ", color = S.config.db.profile.colors.nature},
        right = {text = string.format("%i%%", spellHit.nature), color = S.config.db.profile.colors.nature}
    }
end
...
function S:UpdateManaRegen(entry, regen)
    ...
    entry.tooltip4 = {
        ...
        right = {text = string.format("%i", regen.base * 5), color = S.config.db.profile.colors.highlightFont}
    }
    entry.tooltip5 = {
        left = string.format("%s While Casting:", MANA_REGEN),
        right = {text = string.format("%i", regen.casting * 5), color = S.config.db.profile.colors.highlightFont}
    }
end
...
function S:UpdateBlock(entry, block, blockValue)
    ...
    entry.tooltip3 = " "
    entry.tooltip4 = {left = BLOCK_CHANCE .. ": ", right = {text = string.format("%.2f%%", block), color = S.config.db.profile.colors.highlightFont}}
    entry.tooltip5 = {left = ITEM_MOD_BLOCK_VALUE_SHORT .. ": ", right = {text = string.format("%i", blockValue), color = S.config.db.profile.colors.highlightFont}}
end
...
function S:UpdatePaperDollFrame()
    ...
    if S.Stats.itemLevel then
        if S.Stats.itemLevel >= S.config.db.profile.itemLevels.legendary then
            itemLevelCategory = 5 -- legendary
        elseif S.Stats.itemLevel >= S.config.db.profile.itemLevels.epic then
            itemLevelCategory = 4 -- epic
        elseif S.Stats.itemLevel >= S.config.db.profile.itemLevels.rare then
            itemLevelCategory = 3 -- rare
        elseif S.Stats.itemLevel >= S.config.db.profile.itemLevels.uncommon then
            itemLevelCategory = 2 -- uncommon
        elseif S.Stats.itemLevel >= S.config.db.profile.itemLevels.common then
            itemLevelCategory = 1 -- common
        else
            itemLevelCategory = 0 -- poor
        end
        local r, g, b = GetItemQualityColor(itemLevelCategory)
        PaperDollFrame.CharacterStatsPane.ItemLevel.Value.Text:SetText(Addon:Hex(r, g, b) .. S.Stats.itemLevel .. "|r")
    end
