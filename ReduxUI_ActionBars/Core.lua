...
function AB:Initialize()    
    ...
    AB:SecureHook("MultiActionBar_Update", AB.UpdateAll)
...
function AB:CreateMicroButtonAndBagsBar()
    ...
    frame.Texture:SetTexture(R.media.textures.MainMenuBar)
    ...
    frame.TextureEndCap:SetTexture(R.media.textures.MainMenuBar)
    ...
    if R.IsClassic then
        ...
        MainMenuMicroButton.PerformanceBar.Texture:SetTexture(R.media.textures.PerformanceBar)
    end
...
function AB:UpdateMainMenuBarTextures()
    ...
    if config.theme == AB.themes.Default then
        ...
        AB.bars.MainMenuBar.LeftEndCap:SetTexture(R.media.textures.MainMenuBar)
        ...
        AB.bars.MainMenuBar.RightEndCap:SetTexture(R.media.textures.MainMenuBar)
        ...
        local texture = R.media.textures["MainMenuBar_" .. config.theme]
        if config.theme == AB.themes.Faction then
            if UnitFactionGroup("player") == "Horde" then
                texture = R.media.textures.MainMenuBar_Horde
            else
                texture = R.media.textures.MainMenuBar_Alliance
            end
        end
...
function AB:AddSystemInfo(tooltip)
    ...
    if R.SystemInfo.bandwidth ~= 0 then
        tooltip:AddDoubleLine("Bandwidth", format(bandwidthString, R.SystemInfo.bandwidth), labelColor[1], labelColor[2], labelColor[3], valueColor[1], valueColor[2],
                              valueColor[3])
    ...
    if R.SystemInfo.cpuProfiling then
        for i = 1, #R.SystemInfo.cpuTable do
            ...
            if cpu and IsAddOnLoaded(cpu[1]) then
                ...
                if mem then
                    tooltip:AddDoubleLine(cpu[2], format(cpuAndMemoryString, cpu[3], R.SystemInfo:FormatMemory(mem)), labelColor[1], labelColor[2], labelColor[3], red,
                                          green + .5, 0)
                else
